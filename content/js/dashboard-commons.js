...
function formatDuration(duration, spaced) {
    ...
    var formatArray = [];
    if (days > 0)
        formatArray.push(formatUnit(days, " дни", spaced));

    if (hours > 0)
        formatArray.push(formatUnit(hours, " час", spaced));

    if (minutes > 0)
        formatArray.push(formatUnit(minutes," мин", spaced));

    if (seconds > 0)
        formatArray.push(formatUnit(seconds, " сек", spaced));

    if (duration > 0)
        formatArray.push(formatUnit(duration, " мс", spaced));
...
function getElapsedTimeLabel(granularity) {
    return "Время теста (агрегация: " + formatDuration(granularity) + ")";
}
